...
def preProcessData(date_f,input_f,output_f):
    preProcessDataList = input_f + output_f  # Combine input features and output feature
    preProcessDataList = list(set(preProcessDataList))
...
with col1:
    ...
    if uploaded_file is not None:
        ...
        with st.container(border=True):
            ...
            output_f = st.multiselect(label="Output Feature (Y):",options=[element for element in list(df.columns) if element != date_f])
            st.divider()
